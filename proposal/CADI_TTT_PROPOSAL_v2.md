...
| **F5** | 0.8 | 0.2 | 积分 | 0.03 | 是 | **完成**（与 B 同一滤波器，DC 增益 1；先验 59.5、终值 94.0、×0.7 4.0） |
| **F6** | 0.9 | 0.3 | 积分 | **0.07** | 是 | **完成**（F1 先验、σ_exec 0.048；终值 99.5、×0.7 38.3、被骗代价 +2.3；E4 的全面改进版） |
| **F7** | 0.8 | 0.2 | 积分 | **0.10** | 是 | **完成**（F5 先验、σ_exec 0.033 = B；终值 99.5、×0.7 28.7、jerk 0.101 = 3.2× B：可观测状态使策略输出白化、抵消滤波） |
...
**训练 seed 复现（完成，09-08 14:12，5 个训练 seed）**：

| 臂 | seed 42 | seed 43 | seed 44 | seed 45 | seed 46 | 均值 ± sd |
|---|---|---|---|---|---|---|
| E4 接口 (0.9, 1.0, σ=.03) | 95.0 | 98.0 | **77.5** | 97.0 | 95.0 | 92.5 ± 8.5 |
| E5 原版 σ=.03 | 93.0 | 92.5 | 90.5 | 91.0 | 93.5 | 92.1 ± 1.3 |

§1.1 单 seed 的「E4 95 vs E5 93」在五个训练 seed 下变成「92.5 ± 8.5 vs 92.1 ± 1.3」（样本 sd）：接口臂的微调结果**方差大一个量级**，seed 44 部分崩溃（77.5）。这与 H9 的倒 U 一致——E4 的 σ_exec = 0.069 靠近可训性甜点的右缘（E3 的 0.229 已崩），逐 seed 的运气决定它落在哪一侧；原版 σ_exec = 0.030 在甜点中央、逐 seed 稳定。轴 2 因此需要报告**均值与方差**两项。seed 45/46 约 09-08 与 09-09 完成。
...
一个对等性问题随之暴露：低通 B/F4 的 23.5% 是**未重训**的原版先验被插入滤波器后的成绩，而积分点的先验都是在重标注数据上重训的。为分离「重训 + 状态可观测」与「DC 增益」，07:43 追加 **F5 = (0.8, 0.2) 积分 + obs_cmd + 重训先验**——与 B 是同一个滤波器（DC 增益 1），裁剪率 3.7%。若 F5 的先验接近 F1（≈58）而非 B（23.5），则轴 1 上的差异全部归于重训与状态可观测，与 DC 增益无关。**读数（09-06 中午）：F5 BC = 59.5%**，与 F3 相同——轴 1 上「导数接口」没有独有效应，DC 增益无关；效应来自重训先验与可观测滤波器状态。

**F4 完成（终值 77.5%）与其部署评测（`eval_point.sh`，3 评测 seed）**：同一个低通滤波器，训练 σ 从 0.10（B）降到 0.03（F4），终值 96.0 → 77.5，增益 ×0.7 鲁棒性 41.0 → **1.0%**，延迟 2：87.0 → 64.7，触发式补偿恢复 +55.7 → +39.7，被骗代价 −5.3 → −18.3；最大单步 0.14（全图最平滑）。低通把探索噪声方差压到 $(1-\alpha)/(1+\alpha)=0.11$ 倍，σ=0.03 下几乎没有探索——这是积分器噪声放大（P1）的镜像。

**H9（09-06 17:50 预注册）：轴 2 与轴 4 由滤波器的噪声增益决定，而非结构**。定义训练期执行端探索噪声 $\sigma_{\text{exec}}=s\,\sigma/\sqrt{1-\lambda^2}$（P1 稳态公式；原版 $=\sigma$）。已有点：

| 点 | (λ, s, σ) | σ_exec | 终值（轴 2） | 增益 ×0.7（轴 4） |
|---|---|---|---|---|
| F4 | (0.8, 0.2, 0.03) | 0.010 | 77.5 | 1.0 |
| E5 | 原版 0.03 | 0.030 | 93.0 | 3.7 |
| B | (0.8, 0.2, 0.10) | 0.033 | 96.0 | 41.0 |
| E4 | (0.9, 1.0, 0.03) | 0.069 | 95.0 | 28.3 |
| A | 原版 0.10 | 0.100 | 99.5 | 81.3 |
| E3 | (0.9, 1.0, 0.10) | 0.229 | 12.5（崩溃） | — |
| C | (1.0, 1.0, 0.10) | ∞ | 24.5（崩溃） | — |

轴 4 随 σ_exec 近似单调（唯一交换是 B/E4），轴 2 呈倒 U（0.03–0.10 为甜点，两端崩）。纯积分器（λ=1）没有稳态，按 episode 末随机游走 std $s\sigma\sqrt{T}$（T=400）计：E1 0.6、C 2.0。

**F 系列训练终值与部署评测（09-07 03:00；F2/F3 三评测 seed 完整，F1 部分，F5 训练中）**：

| 点 | (λ, s) | σ_exec | 轴1 先验 | 轴2 终值 | 轴3 最大单步 / P3 界 | 轴4 ×0.7（预注册区间） | 轴4 延迟 2 | 轴5 触发恢复 / 被骗代价 | 校准相关 |
|---|---|---|---|---|---|---|---|---|---|
| F1 | (0.9, 0.3) | 0.021 | 58.5 | 94.0 | 0.30 / 0.4 | **15.0**（5–15 ✓） | 61.0 | +72 / **+5.0** | 0.89 |
| F2 | (0.7, 1.0) | 0.042 | 54.0 | 98.0 | 0.60 / 1.3 | **38.7**（25–40 ✓） | 80.0 | +58.7 / −21.3 | 0.93 |
| F3 | (0.95, 0.5) | 0.048 | 59.5 | 99.0 | 0.43 / 0.55 | **45.7**（30–45，超 0.7） | 84.0 | +48.7 / −11.0 | 0.85 |
| F4 | (0.8, 0.2) 低通 | 0.010 | 23.5 | 77.5 | 0.14 / 0.4 | 1.0 | 64.7 | +39.7 / −18.3 | 0.80 |
| E4 | (0.9, 1.0) | 0.069 | 49.0 | 95.0 | 0.79 / 1.1 | 28.3 | 87.3 | +62.7 / −11.0 | 0.75 |
| E5 | 原版 .03 | 0.030 | 38.0 | 93.0 | 0.67 / 2 | 3.7 | 80.0 | +40.7 / −19.3 | 0.75 |
| A | 原版 .10 | 0.100 | 38.0 | 99.5 | 0.68 / 2 | 81.3 | 91.0 | +17.0 / −39.3 | 0.81 |
| B | (0.8, 0.2) 低通 | 0.033 | 23.5 | 96.0 | 0.20 / 0.4 | 41.0 | 87.0 | +55.7 / −5.3 | 0.93 |

**H9 读数**：预注册区间三中三（F3 超上沿 0.7 pp，在 3 seed 噪声内）；F5 待。但曲线**不严格单调**：E5（σ_exec 0.030 → 3.7%）低于 F1（0.021 → 15%），E4（0.069 → 28%）低于 F2/F3（0.042/0.048 → 39/46%）。偏离趋势的正是当初定义整个故事的两个点——P0b 里 E4 对 E5 的「+25 pp 接口优势」，在 H9 曲线上是两个都低于趋势的点之差；而同一 σ=0.03 家族里 (0.7, 1.0) 与 (0.95, 0.5) 这两个配置在相近 σ_exec 下都比 E4 更鲁棒。H9 的标量形式解释了主趋势（F4 → F1 → F2/F3 → A 单调），残差里有第二个因子，候选：先验质量（E4 的 s=1 标签最小、先验最差）或原版/滤波之别（E5 是 σ=0.03 家族唯一的无滤波点）。**轴 2 的倒 U 成立**：F1 94.0（σ_exec 0.021，靶心左侧）< F2 98.0 < F3 99.0 ≈ A 99.5。

**轴 3 沿 P3 界排列**：最大单步 F4 0.14、B 0.20、F1 0.30（界 0.4）；F3 0.43（0.55）；F2 0.60（1.3）；A/E5 0.67（2）；E4 0.79（1.1）。**F1 = (0.9, 0.3) 是全图最均衡的点**：先验 58.5（第二）、最大单步 0.30（积分点中最小）、触发式补偿恢复 +72（全图最高）、被骗代价 +5.0（唯一非负；1 seed 待补）、校准相关 0.89——代价是 σ_exec 只有 0.021，零适应鲁棒性差（15%）。它正是 E4 应该选的点：把 s 从 1.0 降到 0.3 同时改善了先验、平滑、信号与适应四个轴，只让零适应鲁棒性从 28 降到 15，而后者本可以用更大的 σ 训练弥补（F1 在 σ=0.07 时 σ_exec ≈ 0.048，落在甜点）。

**H10（09-07 03:40 预注册）：轴 4 的第二因子是先验质量**。σ=0.03 家族内按（先验，σ_exec）→ ×0.7 鲁棒性：E5（38, .030）→ 3.7；F1（58.5, .021）→ 14.5；E4（49, .069）→ 28；F2（54, .042）→ 39；F3（59.5, .048）→ 46；E1（43.5, 0.6）→ 45。两个因子都差的 E5 最低，都好的 F2/F3 最高，各占一项的 F1/E4 居中——与「鲁棒性 ≈ f(σ_exec) + g(先验)」一致。两个在跑的点可以判别：**F5**（先验 59.5，σ_exec 0.010）——仅 H9 预测 ≤5%，两因子预测 8–15%；**F6**（先验 58.5，σ_exec 0.048）——仅 H9 预测 30–45%（同 F3），两因子预测 **≥50%**。若 F6 ≥ 50 且 F5 ≥ 8，H10 成立，图的轴 4 需要两个坐标；若 F6 落在 30–45，H9 的标量形式足够、E4/E5 的偏离归于噪声或第三因子。

**F5 完成（09-07 14:58）：H10 在 F5 上不成立；(0.8, 0.2) 处的 2×2 补齐一角**。F5 = 低通滤波器 + 重训先验 + 状态可观测（σ=.03，σ_exec 0.010）：终值 94.0、无漂移 94.7、增益 ×0.7 **4.0%**、延迟 2 73.3、最大单步 0.26（P3 界 0.4）、触发式恢复 **+79.0**（全图最高）、被骗代价 −5.7、校准相关 **0.94**（全图最高）。×0.7 的 4.0% 落在仅 H9 预测（≤5%）内、低于两因子预测（8–15%）；与同滤波器同 σ_exec 的 F4（先验 23.5）比只高 3 pp——**先验质量在零适应鲁棒性上几乎不起作用**，H10 的 F5 检验偏向 H9 标量形式。但先验对**可训性**作用巨大：同滤波器同 σ 下 F5 94.0 vs F4 77.5（+16.5 pp）。F5 与 B 是同一滤波器的两个角（B：原版先验、σ=.10 → 先验 23.5、终值 96、×0.7 41、校准 0.93）：B 的 σ=0.10 买到鲁棒性，F5 的重训买到先验/信号/适应。**F7 = F5 的先验在 σ=0.10 下微调**（σ_exec 0.033 = B）已于 14:59 启动补齐 2×2：仅 H9 预测 ×0.7 ≈ 41%（同 B），两因子预测 > 50%。

**F6 完成（09-07 18:14）：H10 死，H9 标量形式成立；F6 是 E4 的全面改进版但仍不支配 A**。F1 的先验在 σ=0.07 下微调（σ_exec 0.048）：终值 **99.5**、无漂移 99.0、增益 ×0.7 **38.3**（49/36/30）——落在仅 H9 的预注册区间 30–45，未达两因子的 ≥50；结合 F5（4.0，同样落在仅 H9 区间），**先验质量不进入零适应鲁棒性**，H10 撤回。F6 其余读数：观测噪声 σ=.05 → 89（全图最高；E4 78、A 待）、延迟 2 → 67.3、最大单步 0.33（P3 界 0.4）、触发式恢复 +56、被骗代价 **+2.3**（无伤害）、上游/下游 σ_ε=0.5 适应噪声 → 97.5/98.5（无伤害）。

F6 相对原接口 E4 在**除延迟外的每个轴**上更好（先验 +9.5、终值 +4.5、×0.7 +10、观测噪声 +11、最大单步 0.33 vs 0.79、被骗代价 +2 vs −11），是 (λ, s, σ) 三个旋钮各调一格的结果——这是图的建设性结论：**E4 的三个选择（s=1、σ=0.03、λ=0.9）中前两个都错了**，s=1 是对先验对齐的误判，σ=0.03 是接口噪声放大逼出来的、又被 s=1 放大。但 F6 仍不支配强原版 A：零适应鲁棒性 38 vs 81、延迟 67 vs 91；它赢在被骗代价（+2 vs −39）、平滑（0.33 vs 0.68）、先验（58.5 vs 38）。

**延迟鲁棒性沿 s 反向**（意外，待解释）：σ=.03 积分点按 s 排：s=.2 F5 73、s=.3 F1 75、s=.5 F3 84、s=1.0 E4 87/F2 80；F6（s=.3，σ=.07）67 更低。s 小→每步指令增量小→2 步延迟下积分器继续累加过时指令造成的超调应更小，数据却相反；候选解释是 s 小时策略输出 |u| 更大、更依赖每步反馈，延迟破坏的正是这条反馈。列为轴 4 的第二个子轴，不再合并进「鲁棒性」。

**H9 残差：训练 seed 方差已排除，E5 可由噪声颜色解释，E4 仍是唯一异常点**（09-07 19:00）。seed 43/44 的 E4/E5 checkpoint 在 ×0.7 下的冻结评测（`run_seed_robust.sh`）：E4 seed 43–46 → 26.7 / 33.7 / 13.3 / 27.7（seed 42：28.3；五 seed 均值 **25.9 ± 7.5**）；E5 → 8.7 / 3.3 / 1.3 / 4.7（seed 42：3.7；均值 **4.3 ± 2.7**）。两臂的偏低**跨训练 seed 稳定**，不是运气。

接着检验噪声**颜色**：σ_exec 是稳态 std，但对策略有意义的扰动是**一个动作块（4 步）内累积的位移误差**——白噪声（原版）4 步只累积 $2\sigma$，红噪声（λ=0.8–0.95）4 步几乎同向累积到 $3.5\text{–}3.9\,\sigma_{\text{exec}}$。按块尺度 std 重排（10 个有稳态的点）：

| 点 | σ_exec | 块尺度 std | ×0.7 |
|---|---|---|---|
| F4 / F5 | 0.010 | 0.035 | 1.0 / 4.0 |
| **E5 原版** | 0.030 | **0.060** | 3.7 |
| F1 | 0.021 | 0.078 | 12.0 |
| B | 0.033 | 0.117 | 41.0 |
| F2 | 0.042 | 0.138 | 38.7 |
| F6 / F3 | 0.048 | 0.181 / 0.186 | 38.3 / 45.7 |
| A 原版 | 0.100 | 0.200 | 81.3 |
| **E4** | 0.069 | **0.258** | 28.3 |

Spearman（10 点）：σ_exec 0.76，块尺度 0.76；去掉 E4 后分别 0.83 与 **0.92**。块尺度 std 把 E5 放回了趋势线（0.06 的块尺度扰动与 F5 相当，成功率也相当）——**原版策略在 σ=0.03 下学不到纠偏，是因为白噪声在一个决策周期内几乎不产生可见位移**；这也解释了为什么原版需要 σ=0.10 才鲁棒（A 的块尺度 0.20），而 B 用 σ=0.10 加低通只得到 0.117 却已有 41%。E4 在两个度量下都是唯一离群点（块尺度 0.258 是所有有稳态点中最大的，鲁棒性却只有 28）；已排除的解释：先验质量（F5/F6）、训练 seed（43/44）、噪声尺度与颜色（本段）。**未排除**：s=1 使策略输出 |u| 只有 0.069、几乎全部状态信息由 obs_cmd 的 $c$ 承载，增益漂移下 $(c,\ \text{eef})$ 不再一致 → 分布外——这可用「s=1 但 λ=0.7 的 F2 也是 s=1 却不离群（|u| 0.082）」部分反驳，留作论文的公开问题。**H9 的论文表述**：块尺度执行噪声解释 σ=.03/.07/.10 三个家族 10 个点中 9 个的零适应鲁棒性排序（ρ=0.92），原接口配置 E4 是唯一未解释的异常。

**F7 完成（09-08 14:00）与 5 训练 seed 复现结束——图上 12 个点齐了**。F7 = F5 的先验在 σ=0.10 下微调（(0.8, 0.2)、重训先验、状态可观测；σ_exec 0.033 = B）：终值 99.5、无漂移 99.3、增益 ×0.7 **28.7**（H9 预测 ≈41，同 B；**低于趋势**）、延迟 2 86.7、最大单步 0.29、jerk **0.101**（B：0.032）、触发式恢复 +19.7、被骗代价 −23.3、校准相关 **0.73**（全图最低）。与 B 是同一滤波器、同一 σ，唯一差别是策略能否看见滤波器状态（以及随之必需的重训先验）——F7 在平滑、信号、适应三个轴上全面差于 B，只在先验（59.5 vs 23.5）与终值（99.5 vs 96）上更好。

**这对配对揭示了「导数接口」的结构本质（09-08，`analyze_design_space.py` 附表）**。对 11 个有冻结 rollout 的点算策略输出与执行指令的统计：

| 类别 | 点 | 策略输出自相关 ac1(u) | 执行自相关 ac1(c) | 执行 jerk | 执行谱指数 β_c |
|---|---|---|---|---|---|
| 原版 | A / E5 | 0.80 / 0.77 | 同左 | 0.175 / 0.161 | 1.48 / 1.76 |
| 滤波器状态**不可观测** | B / F4 | 0.81 / 0.78 | **0.99 / 0.99** | **0.032 / 0.026** | **3.04 / 2.85** |
| 滤波器状态**可观测** | F5 / F7 / E4 / F1 / F6 / F2 / F3 | **0.31 / 0.25 / −0.14 / −0.11 / 0.03 / 0.40 / −0.11** | 0.93–0.96 | 0.073–0.134 | 1.78–2.25 |

看不见滤波器状态的策略输出与原版一样平滑（ac1 ≈ 0.8），滤波器在其上叠加真正的额外平滑（jerk 降到原版的 1/5–1/6，β_c 提到 ≈3）。看得见状态的策略输出**近白甚至反相关**（符号翻转率 0.67–0.73 vs 0.60），执行 jerk 是不可观测配对的 **2.8–3.2 倍**（F4→F5、B→F7），执行谱指数只到 1.8–2.2——**E4 的 β_c = 1.83 与原版 E5 的 1.76 几乎相同**。这不是 RL 学出来的：闭环重标注的导数标签 $u_t=(a_t-\lambda c_{t-1})/s$ 按定义是示教指令序列的**新息**（对 λ 接近 1 的滤波器近白），积分器再把它染回去，一来一回执行谱回到示教本身的谱。**推论**：P1 的谱整形只作用于叠加在 $u$ 上的**探索噪声**（P0a 的随机 rollout 中 β̂ 从 0.9 抬到 2.1–2.5 是对的），不作用于策略的**意图轨迹**；而低通作用于一个本就平滑的位置域策略，两者都被平滑。「导数接口更平滑」这一常见主张，在**确定性执行**上不成立（jerk 0.07–0.13 vs 原版 0.16–0.18，只降 20–55%；低通降 80%），在**噪声传播**上成立。这也解释了轴 5：近白的执行指令让残差信号更难辨识（F7 校准相关 0.73 vs B 0.93）。

**5 训练 seed 复现完成（09-08 14:12）**：E4 = 95.0 / 98.0 / 77.5 / 97.0 / 95.0（**92.5 ± 8.5**），E5 = 93.0 / 92.5 / 90.5 / 91.0 / 93.5（**92.1 ± 1.3**）。终值均值持平、方差差 6.5 倍；增益 ×0.7 鲁棒性五个 seed E4 25.9 ± 7.5 vs E5 4.3 ± 2.7——两者都稳定低于 H9 趋势线，且 F7（另一个可观测状态 + 大 σ 的点）也低于趋势（28.7 vs B 41）。**轴 4 的第二因子有了新候选**：可观测状态的策略依赖 $c$，增益漂移下 $(c, \text{eef})$ 的一致性被破坏，属分布外——这在 obs_cmd 点里应普遍成立，但 F2/F3/F6 在趋势上、E4/F7 在趋势下，还差一个能区分它们的变量（候选：|u| 的相对幅度——E4 0.09、F2 0.10 极小，F7 0.40 极大；两端都偏离）。留作论文的公开问题，与 §1.8h 的延迟可辨识性一起列入「状态可观测的代价」。

**对论文的含义**：(1) 图上没有任何一个点在五个轴上同时占优，权衡沿两条可解释的轴展开——σ_exec 决定可训性与零适应鲁棒性（H9 主趋势），P3 界 $(1-\lambda)+s$ 决定平滑与误适应伤害；(2) 「导数接口」= 高 DC 增益 + 状态可观测这两项设计，在轴 1 上的效应已被 F5 的先验（59.5 = F3）归零，在其他轴上没有独立于 (λ, s, σ) 的效应；(3) **下一个训练点应是 F1 的 σ=0.07 版本**（F6：(0.9, 0.3, σ=.07)，σ_exec 0.048）——若它在 ×0.7 下达到 40–50% 且保住 F1 的其他四轴，图就有了一个「推荐配置」，论文从纯负结果变成有建设性结论的设计指南。

![H9 检验图：可训性与增益鲁棒性对 σ_exec（灰条 = 待评测点的预注册区间）](figures/DS_h9.png)
**对待评测的四个点的预测（写死）**：F5 (0.8, 0.2, .03) σ_exec = 0.010 → ×0.7 鲁棒 ≤ 5%、终值 ≤ 85；F1 (0.9, 0.3, .03) σ_exec = 0.021 → 5–15%；F2 (0.7, 1.0, .03) σ_exec = 0.042 → 25–40%；F3 (0.95, 0.5, .03) σ_exec = 0.048 → 30–45%。若四点落入区间，H9 成立，图的轴 2/4 可用一条标量曲线概括，「结构」在这两个轴上出局；若 F5 ≫ 5%（状态可观测在低噪声下仍鲁棒）或 F1 ≫ F2（s 小反而更鲁棒），H9 死，结构效应存在。**协议含义**：「匹配探索预算」应匹配 σ_exec 而非策略侧 σ——§1.1 的七臂 kill test 用策略侧 σ 匹配（E4 0.069 vs E5 0.030），其实没有匹配。
...
### 1.11 论文初稿 v0.1 → v0.2 与 ICLR 口径审稿（2026-09-08）

**初稿**：`paper/main.tex`（IEEEtran，8 页）。**审稿**（子 agent，只评故事不评实验规模，`paper/REVIEW_iclr_story_v0.1.md`）：故事分 **5/10**（ICLR 主会边缘以下），按建议重构后预期 6–7。核心意见：**论文的真命题已在公式里但从未说出**——把重标注 $u_t=(a_t-\lambda c_{t-1})/s$ 代回 $c_t=\lambda c_{t-1}+s u_t$ 得 $c_t=a_t$（不触发裁剪时恒等）。状态可观测的导数接口是位置策略的**精确重参数化**（条件于上一步指令、目标按 $1/s$ 缩放），对意图是恒等映射，只对加在 $u$ 上的扰动（探索噪声、模型误差、适应修正）做滤波。全部实测现象都是它的推论：先验提升 = 上一步指令条件 + 目标归一化；「白化」= 示教按 λ 预白化（闭式 lag-1 自相关 $(\phi-\lambda)(1-\lambda\phi)/(1+\lambda^2-2\lambda\phi)$）；上游修正被当作扰动抵消；**平滑意图与可训练的导数先验互斥**。

**三处事实错误（已在 v0.2 修正）**：摘要的 ρ=0.92「十点」实为去掉 E4 的九点（十点 0.76）；贡献 3(a) 与 §VI-D 就 E4 脆弱性是否被解释自相矛盾；「不比示教更平滑」未测示教谱。**补测**（数据集，零 rollout）：示教位置指令 φ=0.91、β=1.93、jerk 0.107；重标注标签 ac1 在训练前已是 −0.08（λ=.9）/ +0.35（λ=.7，预测 0.354）/ −0.14（λ=.95）/ +0.24（λ=.8），微调后策略复现之——「不是 RL 学出来的」有了直接证据；可观测接口的执行 jerk 0.07–0.13 ≈ 示教 0.107，原版 0.16–0.18 比示教更抖（模型误差未滤），只有不可观测低通（0.03）平滑了意图本身。

**v0.2 已做**：命题 1（恒等式）+ 四条推论进入 §III；摘要/贡献/§VI-C/§VI-D 按实测改写；「谱指数」改为带限拟合并给出转折频率。**未做（需决定）**：按审稿 §5 的 ICLR 版重构——标题改为陈述式（*The Policy Inverts the Filter: Derivative Action Spaces Reshape Exploration, Not Behaviour*），结果按「意图通道 vs 扰动通道」而非五轴组织，协议与自我修正史压入附录，相关工作补动作空间/平滑正则/有色探索/噪声注入/因果混淆/残差修正六条线。

**补的逻辑缺口实验（09-08 21:05 启动，仅 BC）**：λ=0 处 2×2 角点——C1 (0,1) 无 obs（重训本身）、C2 (0,1)+obs（上一步指令条件）、C3 (0,0.6) 无 obs（目标缩放，裁剪 2.7%）、C4 (0,0.6)+obs（两者）。预注册：若 C2 ≈ F 系列（≈55–60）而 C1 ≈ 原版（38），先验提升归于条件；若 C3 明显高于 C1，缩放独立起作用；C4 应最高。审稿建议的另两个检验待排：BC 期 c_{t-1} 随机丢弃（检验 E4/F7 的 copycat 型因果混淆）、改变动作块长度 H（检验块尺度噪声机制）。

### 1.12 第二轮审稿与论文 v0.4（2026-09-09）

按 research-refine / kill-argument 流程：写故事卡（round-0，"中介优先"：接口只有一个杠杆）→ 独立审稿人写最强拒稿理由并逐点防守（`paper/refine-logs/round-1-review.md`）。裁决 5/10，核心反驳成立：**同 $\sigma_{\text{chunk}}$ 下可观测状态的点系统性低于不可观测的点**（B 41 vs F7 28.7，同 λ、s、σ；A 81 vs F3/F6 38–46），差距随噪声单调增大（0.035 处 0、0.12 处 12、0.18–0.26 处 30–53 pp），E4/F7 不是离群点而是第二个发现。审稿推荐的**三量分解**（恒等式 ⇒ 接口只能改目标缩放、噪声汇率、上一步指令条件；图给每项定价）是唯一在我们自己的表上为真的论点，已采纳为 v0.4 主线（标题 *What an Action Interface Can Change: An Identity, a Noise Exchange Rate, and a Conditioning Cost*）。同时修正：鲁棒性主张限定为执行器增益漂移；延迟作为陈述性负结果；计数（11/10 点，ρ 0.74/0.90 按 $\sigma_{\text{chunk}}$）；预注册表进附录（F7 失手 = 条件代价）；可训性改为"平台 + 先验决定的低边"；E3/C 先导点进表；去除引言中的项目编年与辩护句。

**决定性实验（09-09 01:51 启动）**：R1 = 原版 + 观测中加 $c_{t-1}$（C2 先验）在 σ=0.10 微调（$\sigma_{\text{chunk}}$=0.20 = A）——条件代价假说预测 ×0.7 从 81 掉向 40，重训假说预测留在 81；R0 = 不观测的重训对照；R2 = 原版 σ=0.13（$\sigma_{\text{chunk}}$=0.26 = E4）；R3 = 低通 (0.8,0.2) σ=0.17（=A）。R2/R3 预测 ≥75。
...
3. **发现**：(a) 训练期**执行端**探索噪声（块尺度、颜色感知）解释零适应漂移鲁棒性的主排序（ρ=0.92，10 点去 E4），并给出可训性的倒 U；高 DC 增益接口结构性地压低可用 σ（P1 的双刃剑）；(b) 泄漏积分 ≡ 高 DC 增益低通，s 在先验对齐与执行空间信任域之间的权衡有闭式表达，且**先验随 s 减小而改善**（标签量程利用）；(c) **滤波器状态可观测的三重代价**：它是导数先验可辨识的必要条件，但 (i) 使接口参数无法上游适应（观测 $c$ 的策略抵消上游缩放并因拉扯更抖），(ii) 使策略输出白化——闭环重标注的导数标签是示教序列的新息——积分后执行谱回到示教谱，执行 jerk 是同滤波器不可观测配对的 2.8–3.2 倍，「导数接口更平滑」只对探索噪声成立、对意图轨迹不成立，(iii) 白化的指令损害残差信号的可辨识性；(d) 训练 seed 方差：接口臂的微调终值方差是原版的 6.5 倍（92.5 ± 8.5 vs 92.1 ± 1.3，5 seed）。
