...
| **F6** | 0.9 | 0.3 | 积分 | **0.07** | 是 | 训练中（F1 先验、σ_exec 0.048；候选推荐配置；itr 150 = 100%） |
...
**F6 中期（itr 150）：100%**。F1 的先验在 σ=0.07 下微调，itr 30 即 94%、itr 150 达 100%（E4 同期 ≈95、F1 ≈94）——σ_exec 从 0.021 提到 0.048 没有伤可训性，反而进入甜点。终值与评测预计 09-08 凌晨；决定性检验是 ×0.7（仅 H9：30–45；两因子：≥50）。
