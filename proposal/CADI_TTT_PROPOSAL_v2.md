...
| **F7** | 0.8 | 0.2 | 积分 | **0.10** | 是 | 训练中（F5 先验、σ_exec 0.033 = B；补齐 (0.8, 0.2) 的先验 × σ 2×2；09-07 14:59 启动） |
...
**训练 seed 复现（进行中，09-07 19:41 四个 seed；seed 46 约 09-08 晚完成）**：

| 臂 | seed 42 | seed 43 | seed 44 | seed 45 | 均值 ± sd |
|---|---|---|---|---|---|
| E4 接口 (0.9, 1.0, σ=.03) | 95.0 | 98.0 | **77.5** | 97.0 | 91.9 ± 9.6 |
| E5 原版 σ=.03 | 93.0 | 92.5 | 90.5 | 91.0 | 91.8 ± 1.2 |

§1.1 单 seed 的「E4 95 vs E5 93」在四个训练 seed 下变成「91.9 ± 9.6 vs 91.8 ± 1.2」（样本 sd）：接口臂的微调结果**方差大一个量级**，seed 44 部分崩溃（77.5）。这与 H9 的倒 U 一致——E4 的 σ_exec = 0.069 靠近可训性甜点的右缘（E3 的 0.229 已崩），逐 seed 的运气决定它落在哪一侧；原版 σ_exec = 0.030 在甜点中央、逐 seed 稳定。轴 2 因此需要报告**均值与方差**两项。seed 45/46 约 09-08 与 09-09 完成。
