...
**训练 seed 复现（进行中，09-07 05:16 三个 seed）**：

| 臂 | seed 42 | seed 43 | seed 44 | 均值 ± sd |
|---|---|---|---|---|
| E4 接口 (0.9, 1.0, σ=.03) | 95.0 | 98.0 | **77.5** | 90.2 ± 11.1 |
| E5 原版 σ=.03 | 93.0 | 92.5 | 90.5 | 92.0 ± 1.3 |

§1.1 单 seed 的「E4 95 vs E5 93」在三个训练 seed 下变成「90.2 ± 11.1 vs 92.0 ± 1.3」（样本 sd）：接口臂的微调结果**方差大一个量级**，seed 44 部分崩溃（77.5）。这与 H9 的倒 U 一致——E4 的 σ_exec = 0.069 靠近可训性甜点的右缘（E3 的 0.229 已崩），逐 seed 的运气决定它落在哪一侧；原版 σ_exec = 0.030 在甜点中央、逐 seed 稳定。轴 2 因此需要报告**均值与方差**两项。seed 45/46 约 09-08 与 09-09 完成。
