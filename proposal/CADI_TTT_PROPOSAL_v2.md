...
**P1' 首个适应实验已完成（§1.6，3 seed）**：残差驱动的接口增益适应把增益 ×0.7 下的成功率从 28.3% 回到 **91.0%**（触发式），而施加完全相同补偿的原版只从 3.7% 到 44.3%，且原版在无漂移时被误适应打到 40.0%（接口 82.3%）。两臂的 ĝ 估计同样准确——差别在**接口对增益误差的双向耐受**（积分作用）与更可辨识的残差信号。H1/H2/H7 获支持，K1/K2 不触发；触发式适应从可选升为必需。
...
2. **机制 = 对增益误差的双向耐受**。ĝ 的估计噪声本身就是一个随机方向的乘性增益误差。原版对任意方向的增益误差都脆弱（P0b：×0.8 即掉到 43%），所以常开适应在无漂移时把它打到 40.0（−48）；接口靠积分作用两向耐受（H7），同样的误适应只掉到 82.3（−15）。这把「结构性安全壳」从谱整形论证（P1/P2）扩展到了**对适应误差的鲁棒性**——是 P2 有界传播在参数级适应上的经验对应。
...
**对预注册判据的读数**：H1（适应收益 ≥10 pp）在接口上成立（+10.6 / +62.7）；H2（接口伤害更低）成立（误适应伤害 −15 vs −48，jerk 全部条件更低）；H7 获得第二次独立支持；**K1、K2 均不触发**。K3（DSRL 对照）与 5 训练 seed 复现仍待 P2。
...
| H7 | 积分作用抵消执行器增益漂移（P0b 观察） | 5 seeds 下增益 ×0.8 时接口臂（E1/E4 型）比匹配原版高 ≥15 pp；去掉 obs_cmd 的接口臂若同样稳健，则机制归于积分结构而非可观测意图 | 冻结 +36；误适应耐受第二次支持；obs_cmd 消融待做 |
...
| P1 信号与触发 | 神经头级辅助头/残差/密度/共形阈值；**延迟感知估计器**（H4 修正）；H4 单独验证 | 小 | 1 周 |
...
- 已实现（P1'）：`dppo/env/gym_utils/wrapper/adaptive_gain.py`（`AdaptiveGainWrapper`：残差滑窗估计 ĝ、死区/滞回触发、裁剪、校准日志）、`p0/run_p1_batch.sh`、`p0/run_p1_trig.sh`、本地 `analyze_p1.py`。
