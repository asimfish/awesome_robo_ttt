...
**H9 残差：训练 seed 方差已排除，E5 可由噪声颜色解释，E4 仍是唯一异常点**（09-07 19:00）。seed 43/44 的 E4/E5 checkpoint 在 ×0.7 下的冻结评测（`run_seed_robust.sh`）：E4 seed 43 → 26.7、seed 44 → 33.7（seed 42：28.3；三 seed 均值 29.6 ± 3.7）；E5 seed 43 → 8.7、seed 44 → 3.3（seed 42：3.7；均值 5.2 ± 3.0）。两臂的偏低**跨训练 seed 稳定**，不是运气。
