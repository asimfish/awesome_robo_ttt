...
**首批读数（$\sigma_\varepsilon=0.25$，seed 1000）**：成功率四种配置都在无漂移 ±3 pp 内（噪声太小，看不出成功率差异；等 0.5）；jerk 上上游注入明显更平滑——E4-up ×1.02 vs E4-down ×1.48，B-up ×1.11 vs B-down ×3.20（B 的基线 jerk 0.031 极低，比值放大；绝对值 0.034 vs 0.099，仍低于 E4 的任何配置）。方向与 (f) 的预测一致，幅度待 $\sigma_\varepsilon=0.5$ 与 3 seed。
...
| 上游适应噪声的 jerk 增幅 | — | 低 | ≈（1.02 vs 1.11） | §1.8f |
...
**尚未落地、可能改变上表的实验**（在跑）：$\sigma_\varepsilon=0.5$ 的上下游适应噪声（若 E4-up 在成功率上显著优于 E4-down 与 B-up，P2 保住一条经验支撑）；上游补偿的 P1'；过补偿 seed 2–3；B 的常开延迟；5 训练 seed。

**路线建议**（需要你决定）：
- **路线 A（推荐）**：把这项工作写成一篇**受控的负结果研究**——「一个导数动作接口为扩散策略的 RL 微调和部署期适应买到了什么？」贡献：(i) 一套评价动作接口主张的实验协议（匹配预算孪生 + 最强原版 + 滤波器零假设 + 直接机理测量），本方案的每一次自我修正都是协议条目；(ii) 训练探索噪声水平主导部署鲁棒性、而积分型接口结构性地压低可用噪声这一发现；(iii) 泄漏积分 ≡ 高 DC 增益低通的等价性及其对「导数/速度/加速度动作空间」类主张的含义；(iv) 命令谱与自监督残差信号可辨识性的关系。可在现有数据 + 5 seed 复现基础上 2–3 周成稿；目标 RA-L / CoRL 或 ICRA workshop（负结果友好）。
