...
\subtitle{Test-Time Training for Robot Policies —— 88 篇文献调研汇总}
...
\titlepage
\begin{center}
\small 88 篇论文 · 9 大分类 · 英文 PDF + 中文翻译 + 逐篇精读 · 9 大趋势 + 5 条洞见\\
...
\begin{frame}{九大分类：88 篇论文的全景地图}
\small
\begin{tabular}{p{0.24\textwidth}p{0.33\textwidth}p{0.28\textwidth}c}
\toprule
\textbf{分类} & \textbf{核心问题} & \textbf{代表工作} & \textbf{篇数}\\
\midrule
1. TTA/TTT 基础 & 何时有效 / 何时崩溃 / 攻击面 & TTT · TENT · SAR · SHOT · MEMO & 12\\
2. 部署时策略适应 & 适应哪个模块、用什么信号 & PAD · RMA · Neural-Fly · MAML & 10\\
3. TTT 新浪潮 & TTT 变成网络层 / 测试时 RL & TTT-Layers · Titans · LaCT · TTRL & 12\\
4. 机器人/驾驶权重级 TTT & 真机闭环 / 驾驶榜单证据 & RoboTTT · Centaur · TTT-Parkour & 7\\
5. 免权重 Steering & 不改权重的引导/搜索/验证/组合 & DSRL · TOAD · GPC · ORPA & 22\\
6. 2025--2026 前沿 & 可靠性协议 / 算力调度 / PRM & VANE · ELASTIC · DREAM-Chunk & 8\\
7. 免梯度上下文适应 & 演示/历史进上下文，零梯度快适应 & ICRT · LocoFormer · AnyCar & 5\\
8. 失败检测与适应触发 & 什么时候该适应 & Sentinel · FAIL-Detect · SAFE & 3\\
9. TTT 之前的 RL 阶段 & on-policy 为何学崩 / 探索噪声频谱 & DPPO · Q-Chunking · Pink Noise & 9\\
\bottomrule
\end{tabular}
\vspace{6pt}
...
\end{itemize}
\column{0.48\textwidth}
\textbf{System-2 · 免权重 Steering}（分类 5--6）
\begin{itemize}
...
\begin{frame}{分类 1 基础：给机器人划下的三条红线}
\begin{itemize}
...
\begin{frame}{分类 2 策略适应：「适应哪个模块」的风险谱系}
\centering
\small
\begin{tabular}{lllll}
\toprule
...
\begin{frame}{分类 3 新浪潮：TTT 从部署补丁变成网络层}
\begin{columns}[T]
...
\begin{frame}{分类 4 机器人/驾驶 TTT：真机 30Hz 与驾驶榜首}
\small
\begin{columns}[T]
...
\begin{frame}{分类 5 免权重 Steering：设计空间与延迟账本}
\begin{columns}[T]
\column{0.5\textwidth}
\textbf{四象限（可组合）}
\begin{itemize}
...
\begin{frame}{分类 6 前沿七篇：可靠性、接口化与算力调度}
\small
...
\begin{frame}{新增分类 8/9：失败检测器与「TTT 之前的阶段」}
\begin{columns}[T]
\column{0.5\textwidth}
\textbf{8. 失败检测 = 适应的开关}
\small
...
方法 & 失败数据? & 信号\\
\midrule
Sentinel & 否 & 动作块一致性 + VLM\\
FAIL-Detect & \hl{否} & 成功密度 + 共形阈值\\
SAFE & 是(跨任务) & VLA 内部特征\\
\bottomrule
\end{tabular}
\vspace{4pt}

信号来自策略内部、共形预测给阈值保证；\wa{熵类信号抓不到「自信地做错」，触发器需分层。}
\column{0.46\textwidth}
\textbf{9. TTT 站在 RL 管线之上}
\begin{itemize}
\item on-policy 早期学崩的两个正交机制：价值侧 Q 尺度失配（Cal-QL）$\times$ 动作侧探索噪声放大（本仓库源头实验）
\item 探索噪声有最优频谱：Pink Noise $\beta\approx1$；gSDE 生成端平滑；Q-Chunking 分块相关性
\item \hl{导数接口 = 频谱整形器}：积分一次 $\beta{+}2$（白$\to$红，Arm C 崩溃），泄漏积分拉回中间（E3 恢复）——泄漏系数即噪声颜色旋钮
\end{itemize}
\end{columns}
\end{frame}
...
\item \textbf{段级「重建世界再训练」}：TTT-Parkour / MPA——高动态不可重试场景的现实 TTT 形态；粒度谱系补齐帧$\to$chunk$\to$段$\to$任务
\item \textbf{失败检测器成为标配前置件}：Sentinel / FAIL-Detect / SAFE，信号来自策略内部，共形预测保证阈值
\end{enumerate}
\end{frame}
